...
buildIndexes();
RESULT={all:allNames.length,indexed:searchIndex.size,missing:CATEGORIES.filter(x=>!searchIndex.has(x.icon)).length,counts:CATEGORIES.map(x=>(categoryIndex.get(x.id)||[]).length),invalid:normalizeDesign({icon:'bad',bg:'bad',mode:'bad'}),front:composeSvg({...state,shape:'front-cube'}).includes('x="42"')};`;
