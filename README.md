...
たんぽぽ時間割への反映と、時数集計表（Excel）へのコピーを1操作で出す。

| ファイル | 内容 |
|---|---|
| [`docs/weekly-plan.md`](docs/weekly-plan.md) | **設計の正本（v0）。まずここを読む** |
| `prototypes/weekly-plan.html` | 版面と入力操作のプロトタイプ（生成物）。ブラウザで開くだけで動く |
| `prototypes/src/weekly-plan.html` | その元。**直すのはこちら** |
| `prototypes/src/plan-tokens.css` | 週案の色トークン（ノート評価とは別） |
| [`gas/weekly/`](gas/weekly/) | GAS 側。いまは**関門（教職員だけを通す）**まで |

本体は GAS ウェブアプリで作る（理由は docs 1節）。このプロトタイプは**運用の本体ではなく、
刷った紙が教務必携に貼れるかを先に確かめるためのもの**。保存はブラウザ内。

画面は**暗い机の上に白い紙が1枚**という作りにしてある。
**紙の上に出ているものだけが刷られる。** 机の上の操作は何も刷られない。

児童（`【8桁】@kyoiku.edu.nishi.or.jp`）は入れない。
**児童のドメインは教職員のドメイン（`@edu.nishi.or.jp`）を末尾に含む**ので、
`endsWith` で見ると素通りする。`gas/weekly/Gate.gs` が `@` の右側を完全一致で見ている。

```
node gas/weekly/gatecheck.js   関門を手元で確かめる（28件）
```

**次にやること：B5で1枚刷って手帳に当て、余白と倍率を確定する。**
ここが合わないと以降の機能が全部無駄になる。
