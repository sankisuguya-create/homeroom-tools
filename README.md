...
外へ出す先が2つある。どちらも実物に合わせてある。

| | |
|---|---|
| たんぽぽ時間割 | 日ブロックを**1日16行**に揃えてもらう（いま86行 → 81行。**B4 1枚に入れやすくなる**）。交流か在籍かは**担当の行**（交：／た：／支：）で決める |
| 時数集計表 | 空セル込みの1つの矩形にして、左上を選んで1回で貼る |
| 年間行事計画表 | 1行1日の縦長の表を `行事取り込み` シートに貼ってもらう。3か月横並びのままは読ませない |
